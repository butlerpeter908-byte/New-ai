...
# ================= 2. ADVANCED UI CSS (MOBILE OPTIMIZED) =================
...
st.markdown(f"""
    ...
    /* CUSTOM MOBILE MENU BUTTON (Neon Style) */
    ...
    .chat-container {{ display: flex; flex-direction: column; gap: 10px; padding: 10px; margin-top: 50px; }}
    .user-bubble {{
...
if not st.session_state.logged_in:
    ...
    with t2:
        ...
        if st.button("Create Account", use_container_width=True):
            st.session_state.user_db[nu] = np; st.success("Success! Now Login.")
    st.stop()

# ================= 4. PROFESSIONAL SIDEBAR (HIDDEN UNTIL LOGIN) =================
with st.sidebar:
    st.title("🤖 New AI Menu")
    st.markdown(f"User: **{st.session_state.current_user}**")
    st.markdown("---")
    
    # All pages back in sidebar as requested
...
# ================= 5. PAGES CONTENT (ENGLISH) =================
if menu == "💬 Chat":
    st.markdown('<div style="color:white; text-align:center; padding-top:20px;"><h3>🤖 Chat Interface</h3></div>', unsafe_allow_html=True)
    ...
    st.header("📩 Feedback")
    fb = st.text_area("Share your experience with Siddique...")
    if st.button("Submit Feedback"):
        try:
            ...
            st.success("Feedback sent to Siddique's Gmail! ✅")
        except: st.error("Mail service error.")

elif menu == "🛡️ Privacy Policy":
    st.header("🛡️ Privacy Policy")
    st.write("We value your privacy. Your session data is cleared upon logout and we do not share info with third parties.")

elif menu == "📄 Terms & Conditions":
    st.header("📄 Terms & Conditions")
    st.write("1. Do not use for illegal purposes. 2. AI content is for informational use only. 3. Respect the developer's rights.")

elif menu == "👤 About Creator":
    st.header("👤 About Creator")
    st.info(f"This application is crafted with passion by **{CREATOR_NAME}**.")
