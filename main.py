...
audio_data = mic_recorder(start_prompt="🎙️", stop_prompt="🌊", key='final_verified_mic_v14')
st.markdown('</div>', unsafe_allow_html=True)
...
if u_input:
    ...
    # Simple live weather simulation (Can be linked to OpenWeather API)
    ...
    try:
        with st.chat_message("assistant"):
            ...
            # Revised Strict Prompt
            sys_msg = (
                f"You are Pro AI. Today's Date: {curr_date}. Current Time: {curr_time}. Live Weather: {weather_info}. "
                "STRICT RULES: "
                "1. If user asks for 'time', tell the time. "
                "2. If user asks for 'date', tell the date. "
                "3. If user asks for 'weather', provide the live weather info. "
                "4. DO NOT mention date, time, or weather unless specifically asked for each."
            )
